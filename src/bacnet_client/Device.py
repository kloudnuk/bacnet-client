...
class BacnetDevice:
    ...
    def __init__(self, id, addr: str, props: dict, doNormalize=True) -> None:
        if doNormalize:
            self.__properties = OrderedDict(
                sorted(
                    OrderedDict(
                        {p: self.normalize(str(p), props[p]) for p in props}
                    ).items(),
                    key=lambda x: x[0],
                )
            )
    ...
    def __eq__(self, other) -> bool:
        if type(other) == BacnetDevice:  # noqa: E721
            return int(str(self.deviceId).split(",")[1]) == int(
                str(other.deviceId).split(",")[1]
            ) and str(self.address) == str(other.address)
        else:
    ...
    def oct2Address(self, octetString):
        ...
        if len(octets) <= 1:
            ...
            ipString = f"{int(octets[1][1:], 16)}.{int(octets[2][1:], 16)}.{int(octets[3][1:], 16)}.{int(octets[4][1:], 16)}:{int(octets[5][1:] + octets[6][1:], 16)}"  # noqa: E501
    ...
    def normalize(self, property, value):
        try:
            normalized: dict = {"value": "", "type": str(type(value))[18:-2]}
            if property == "restart-notification-recipients":
                ...
                try:
                    normalized["value"] = [
                        {
                            "device": self.oct2Address(
                                v.recipient.recipient.address.macAddress
                            ),
                            "propertyReference": f"{v.monitoredPropertyReference.objectIdentifier}-{v.monitoredPropertyReference.propertyIdentifier}",  # noqa: E501
                            "timeRemaining": str(v.timeRemaining),
                            "covIncrement": str(v.covIncrement),
                        }
                        for v in value
                    ]
                    return normalized
...
class LocalBacnetDevice(Subscriber):
    ...
    def __init__(self) -> None:
        ...
        self.settings: dict = {
            ...
            "objectIdentifier": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[0], "objectIdentifier"
            ),
            "objectName": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[0], "objectName"
            ),
            "vendorIdentifier": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[0], "vendorIdentifier"
            ),
            "tz": pytz.timezone(
                self.localMgr.read_setting(LocalBacnetDevice.__ini_sections[0], "tz")
            ),
            "maxApduLengthAccepted": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[1], "maxApduLengthAccepted"
            ),
            "maxSegmentsAccepted": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[1], "maxSegmentsAccepted"
            ),
            "interface": self.localMgr.read_setting(
                LocalBacnetDevice.__ini_sections[1], "interface"
            ),
        }
